...
export class PostController {
  ...
  @Get()
  async getAllPosts() {
    const data = await this._postService.getAllPostsService();
    return { message: 'Posts found!!', data };
  }

  @Get(':id')
  async getOnePost(@Param('id', ParseIntPipe) id: number) {
    const data = await this._postService.getOnePostService(id);
  ...
  async createPost(@Body() createPostDto: CreatePostDto, @UserDecorator() owner) {
    const data = await this._postService.createPostService(createPostDto, owner);
    return { message: 'Post created!!', data };
  }

  @Put(':id')
  async editPost(@Param('id') id: number, @Body() editPostDto: EditPostDto) {
    const editedPost = await this._postService.editPostService(id, editPostDto);
    return { message: 'Post edited!!', editedPost };
  }

  @Delete(':id')
  async deletePost(@Param('id') id: number) {
    const { affected } = await this._postService.deletePostService(id);
